...
class MCU:
    ...
    def _connect(self):
        if self._non_critical_disconnected:
            ...
            return
        logging.info("starting connect for mcu: %s", self._name)
        ...
        if not config_params["is_config"]:
            ...
            logging.info("not configured, sending config again")
    ...
    def non_critical_recon_event(self, eventtime):
        logging.info("noncritical recon event")
        if self.last_noncrit_recon_eventtime is None:
            self.last_noncrit_recon_eventtime = eventtime
        if eventtime - self.last_noncrit_recon_eventtime > 2:    
            self.recon_mcu()
            self.last_noncrit_recon_eventtime = eventtime
        return eventtime + 0.912
